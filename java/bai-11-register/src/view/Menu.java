...
public class Menu {
    ...
    public void register(Scanner sc, List<User> users) {
        ...
        if (userHandle.findUser(users, username, "username") != null) { //find by username
            System.out.println("Username da duoc dang ky.");
        } else {
            System.out.println("Nhap email:");
            String email = inputControl.getEmail(sc); //Doi thanh inputControl
            if (userHandle.findUser(users, email, "email") != null) { //find by email
                System.out.println("Email da duoc dang ky.");
            } else {
                System.out.println("Nhap mat khau:");
                String password = inputControl.getPassword(sc); //Doi thanh inputControl
                //Tao user moi
                userHandle.addUser(users,username,email,password); // Toan bo method register chi co doan nay lien quan truc tiep den user
            }
        }
    }
    ...
    public void login(Scanner sc, List<User> users) {
        ...
        String username = sc.nextLine();
        if (userHandle.findUser(users, username, "username") != null) { //find by username
            int index = users.indexOf(userHandle.findUser(users, username, "username"));
    ...
    public void forgotPassword(Scanner sc, List<User> users, int index) {
        System.out.println("Nhap email:");
    ...
    public void loginFailed(Scanner sc, List<User> users, int index) {
        int input;

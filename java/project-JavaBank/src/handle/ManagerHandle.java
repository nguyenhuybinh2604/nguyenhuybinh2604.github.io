...
public class ManagerHandle {
    ...
    public void viewBonus(Scanner sc, InputControl inputControl, SummaryHandle summaryHandle,
                          CustomerHandle customerHandle, Map<String, Object> users, List<Product> products) {
...
//        viewListOfStaffs(summaryHandle, staffHandle, users, products);
        ...
        if (activeProducts.size() > 0) {
            ...
            for (Map.Entry<String, Object> entry : users.entrySet()) {
                if (entry.getClass().getSimpleName().equals("Staff"))
                    annualSalaryFund += ((Staff) entry.getValue()).getBasicSalary();
                else if (entry.getClass().getSimpleName().equals("Manager"))
            ...
            double staffBonus = totalBonus - managerBonus;

            // get only staffs with at least 1 active products
            Map<Integer, Summary> staffs = summaryHandle.byStaff(users, activeProducts);
...
//enter effect date
        ...
        public void approveCreditRating () {

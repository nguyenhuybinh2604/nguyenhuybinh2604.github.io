...
public class BTVN_Buoi7 {
    ...
    // Toc do: moi vong de quy thuc hien 4 lan check & 3 thao tac & do dai 2 vong de quy n-1 & n-2
    // Tinh tu duoi len do da luu vao array -> Moi vong de quy mat khoang 4 lan check & 3 thao tac
    ...
    private int recursive_509(int[] nums, int n) {
        ...
        } else return recursive_509(nums, n - 2) + recursive_509(nums, n - 1);
    }


    //https://leetcode.com/problems/to-lower-case/submissions/984252423/
    // Bo nho: stringbuilder & i -> O(n)
    // Toc do: vong for: n*(check i<sb.lenth & (i++) & check char la uppercase & convert) = 4n, + sb.toString = 5n -> ~O(n);
    ...
    // Thay vi tao 01 mang moi va duyet tu duoi len -> tan dung khoang trong n slot cua nums1 va duyet tu tren xuong.
    // Phan tu nums1[i] khong bi ghi de vi de den duoc i thi:
    // Hoac toan bo nums2 da duoc ghep vao,
    // Neu den i ma nums2 van con phan tu -> cac phan tu nums1[i] van lon hon -> tiep tuc duoc day ve sau
    ...
    // Toc do: duyet vong for 1 lan qua n so, moi vong thuc hien khoang 10 phep tinh -> ~O(n)
    public boolean checkIfExist_1346(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            if (halfNum * 2 == arr[i] && doubleMap.containsKey(halfNum)) return true;
                // check ton tai so gap doi
            else if (doubleMap.containsKey(doubleNum)) return true;
                // neu khong tim thay so 1 nua/so gap doi va chua ghi nhan arr[i] -> add vao map de theo doi
    ...
    public boolean isToeplitzMatrix_766(int[][] matrix) {
        ...
        for (int j = 1; j < matrix.length; j++) {
            ...
            if (!Arrays.equals(matrix[j], intArray)) return false;
        }
        return true;
    }
}
